...
class DoubleCosetReduction(SageObject):
    ...
    def igamma(self,embedding = None):
        ...
        # mat = Matrix(ZZ,4,1,self._cached_igamma.list())
        # n = ZZ((mat[0,0]*mat[3,0]-mat[1,0]*mat[2,0]).valuation(self._parent._p)/2)
        # assert prod([Y._character(ZZ((v*mat)[0,0]))/(Y._character(self._parent._p)*n) for v in Y.get_extra_embedding_matrices()]) == 1
...
class BruhatTitsTree(SageObject, UniqueRepresentation):
    ...
    def find_path(self,v,boundary=None):
        ...
        An ordered list of vertices describing the geodesic from ``v`` to
    ...
    def find_geodesic(self,v1,v2,normalized = True):
        ...
        OUTPUT:
          ...
          ordered list of 2x2 integer matrices representing vertices
...
class BTQuotient(SageObject, UniqueRepresentation):
    ...
    - ``Nplus`` - an integer corpime to pNminus (Default: 1). This is the tame level. It need
       ...
       quaternion algebras.
    - ``character`` - a Dirichlet character (Default: 1). Its modulus must divide the product `p N^- N^+`.
    ...
    def dimension_harmonic_cocycles(self,k,lev = None,Nplus = None,character = None):
        ...
        return GammaH(lev*Nplus,kernel).dimension_cusp_forms(k = k) - sum([len(ZZ(lev/d).divisors())*self.dimension_harmonic_cocycles(k,d,Nplus,character) for d in divs[:-1]])
    ...
    def is_admissible(self,D):
        r"""
        Returns whether the set `CM(O)` is nonempty, where `O` is an order
        of discriminant ``D``.
    ...
    def get_SH_points(self,disc,prec):
        p=self._p
        ...
        trd = w.trace()
        nrd = w.norm()
        print 'trd = %s, nrd = %s'%(trd,nrd)
        all_elts =self._find_elements_in_order(nrd,trd)
        return all_elts
    ...
    def _get_hecke_data(self,l):
        ...
        #     print ii,(self.get_extra_embedding_matrices()[0]*Matrix(ZZ,4,1,V[ii])).list()
        #     print ii,map(Zmod(self._p),(self.get_embedding_matrix()*Matrix(ZZ,4,1,V[ii])).list())
    ...
    def _compute_quotient(self, use_formulas = True):
        ...
        while len(V)>0:
            ...
            # if num_verts == total_verts and num_edges == total_edges:
            #     break
