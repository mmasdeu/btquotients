...
class HarmonicCocycleElement(HeckeModuleElement):
    ...
    def values(self):
        r"""
        Returns the values of the cocycle on each of the representatives.
        """
        return [self._F[e] for e in range(self._nE)]
...
class HarmonicCocycles(AmbientHeckeModule):
    ...
    - Marc Masdeu
    """
    def __init__(self,X,k,prec = None,basis_matrix = None, base_ring = None):
        self._k = k
        self._X = X
        ...
        if prec is None:
            ...
            if base_ring is None:
                ...
                self._R = base_ring.extension(pol,pol.variable_name()).absolute_field(name = 'r')
    ...
    def change_ring(self, new_base_ring):
        ...
        if not new_base_ring.has_coerce_map_from(self.base_ring()):
            ...
            return self.__class__(self._X,self._k,prec = self._prec,basis_matrix = self.basis_matrix().change_ring(new_base_ring),base_ring = new_base_ring)
    ...
    def basis_matrix(self):
        ...
        EXAMPLES::
            ...
            ...
            RuntimeError: The number of computed basis elements (11) does not agree with the expectation (10). Consider increasing precision!
        ...
        ::
            ...
            sage: M = HarmonicCocycles(X,4,100) # long time optional - magma
            sage: B = M.basis() # long time optional - magma
    ...
    def __apply_atkin_lehner(self,q,f):
        r"""
        This function applies an Atkin-Lehner operator to a harmonic cocycle
...
class HarmonicCocyclesSubmodule(sage.modular.hecke.submodule.HeckeSubmodule,HarmonicCocycles):
    ...
    def change_ring(self, new_base_ring):
        ...
        if not new_base_ring.has_coerce_map_from(self.base_ring()):
            ...
            return self.__class__(self.ambient_module().change_ring(new_base_ring),self.basis())
...
class pAutomorphicFormElement(ModuleElement):
    ...
    def evaluate(self,e1):
        ...
        p = self.parent().prime()
        u = DoubleCosetReduction(X,e1)
        return self._value[u.label].r_act_by((u.t(1+2*self.precision()))*p**(u.power))
    ...
    def _repr_(self):
        ...
        """
        return 'p-adic automorphic form on '+str(self.parent())

    def values(self):
        r"""
        Returns the values of the form on each of the representatives.
        """

        return [self._value[e] for e in range(self._num_generators/2)]
    ...
    def integrate(self,f,center = 1,level = 0,method = 'moments'):
        ...
        if(method == 'riemann_sum'):
            ...
            for e in E:
                ii += 1
                #print ii,"/",len(E)
                exp = ((R1([e[1,1],e[1,0]]))**wt * e.determinant()**(-wt/2))*f(R1([e[0,1],e[0,0]])/R1([e[1,1],e[1,0]]))
                #exp = R2([tmp[jj] for jj in range(self.parent()._k-1)])
                new = self.evaluate(e).evaluate(exp.truncate(wt+1))
                value += new
        elif(method == 'moments'):
            ...
            for e in E:
                ii += 1
                #print ii,"/",len(E)
                tmp = ((R2([e[1,1],e[1,0]]))**wt * e.determinant()**(-wt/2))*f(R2([e[0,1],e[0,0]])/R2([e[1,1],e[1,0]]))
